...
def visuialize_performance(back_tester, *data):
    ylabel_dict = {
               'fontsize'            : 12,
    ...
    account = back_tester.get_account()[:back_tester.count]
    ax2.plot(account, color='gray')
    ax2.set_ylabel('account', **ylabel_dict)
    for stockID, signal in back_tester.get_signals().items():
    ...
    do_save=True
    if do_save:
        plt.savefig('output/rst.jpg', quality=100)

# visuialize_performance(back_tester, df[['13MA']], df[['34MA']], trading_data[['2380_close']])
